...
while (~finish) %(~finish && hasFrame(vidReader)) 
...
for i=1:height(bbox)
    ...
    starty = tmp_bbox(2);endy = min(tmp_bbox(2)+tmp_bbox(4), hig);
    startx = tmp_bbox(1); endx = min(tmp_bbox(1)+tmp_bbox(3), len);
...
for index=1:height(expected_bboxes)
    ...
    elseif(dx == 0 && dy == 0)
        ...
        if(frames_count > 10)%TODO check if pigeon and remove accordingly
            starty = expected_bboxes(index,2);endy = min(expected_bboxes(index,2)+expected_bboxes(index,4), hig);
            startx = expected_bboxes(index,1); endx = min(expected_bboxes(index,1)+expected_bboxes(index,3), len);
